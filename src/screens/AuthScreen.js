  ...
  async function handleGoogleLogin() {
    ...
    try {
      ...
      const result = await WebBrowser.openAuthSessionAsync(data.url, redirectTo, {
        createTask: false, // Android fix — keeps auth in the same app task so the redirect works
      });

      // After the browser closes, try to extract tokens from the redirect URL
      if (result.type === 'success' && result.url) {
        ...
        if (accessToken && refreshToken) {
          ...
          return; // onAuthStateChange in AppNavigator will handle the switch to Home
        }
      }

      // Fallback — check if Supabase already has a session (set via deep link)
      const { data: { session: existingSession } } = await supabase.auth.getSession();
      if (!existingSession) {
        Alert.alert('Sign In Incomplete', 'Could not complete sign in. Please try again.');
      }
      // If session exists, AppNavigator will detect it and navigate automatically
    } catch (err) {
